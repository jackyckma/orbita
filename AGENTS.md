...
Startup layer only refreshes deps (`pnpm install`). Everything below is non-obvious run/setup context; standard commands live in `README.md`, `docs/site/setup.md`, and root `package.json` scripts.

- **Docker engine is required and is a system dependency (not in the update script).** The dev stack needs Postgres via `docker compose`. If `docker` is missing on a fresh pod, install Docker CE and start the daemon manually (see the Cloud Agent docker-in-docker recipe: `fuse-overlayfs` storage driver in `/etc/docker/daemon.json` + `iptables-legacy`, then run `sudo dockerd` in a background/tmux session). No systemd here, so `dockerd` must be started manually. Give the socket group access (`sudo usermod -aG docker ubuntu` / `sudo chmod 666 /var/run/docker.sock`).
- **Postgres schema bring-up:** `docker compose up -d postgres` builds the full schema on first boot via the `docker-entrypoint-initdb.d` mounts, and `pnpm db:migrate` applies the same migrations to an already-running Postgres (`DATABASE_URL=postgresql://orbita:orbita@localhost:5432/orbita pnpm db:migrate`). Both paths are now locale-independent and cover all migrations. Expected result: 8 tables (`api_keys, sessions, messages, client_memories, notes, note_links, trajectory_events, session_jobs`). Historical gotcha (fixed): the initdb mounts previously used `002b_`-style suffixes that could sort *before* `002_` under some locales, running a dependent migration before its base table existed and aborting init (container exits code 3); the mounts now use strict `NNN_` numbering. If you ever re-hit an init abort, the container still leaves a valid PGDATA — a second `docker compose up -d postgres` serves it, then run `pnpm db:migrate` to finish the schema (all migrations are `IF NOT EXISTS`, safe to re-run).
- **`.env` empty-string URL gotcha:** `apps/orbita-api` (`pnpm dev`) validates config with Zod and empty strings still fail `.url()`. Leave optional URL vars (`ORBITA_SEARXNG_BASE_URL`, `ORBITA_PUBLIC_BASE_URL`) **unset** (omit the line) rather than `VAR=`. `.env.example` ships them as empty `VAR=` which crashes `pnpm dev` at startup — copy it but delete those empty URL lines. The e2e scripts avoid this because they set their own minimal env instead of loading `.env`.
- **Running an agent turn without LLM secrets:** no `MINIMAX_API_KEY`/`ANTHROPIC_API_KEY` is needed to exercise the full session→turn→trajectory path — start the server with `ORBITA_E2E_MOCK=1` and the agent replies with a deterministic `e2e_mock` provider response. Real turns need a MiniMax or Anthropic key in `.env`.
- **API auth headers differ by route:** admin routes (`/v1/admin/*`, `/admin`) use header `x-orbita-admin-token: <ORBITA_ADMIN_TOKEN>` (not `Authorization`). Caller routes use `Authorization: Bearer <api_key>` **plus** `x-orbita-client-id: <client_id>` (the client id must be in the key's `allowed_client_ids`). Session id is returned nested under `session.id`.
- **Verify commands:** L0/lint = `pnpm typecheck` (there is no separate ESLint/Biome — only a stray `eslint-disable` comment). Unit tests = `pnpm -r test`. Tier A e2e (Docker Postgres + built API + mock LLM) = `bash scripts/e2e-tier-a.sh` (self-contained; builds and starts its own API on :3099). Tier B needs a live `MINIMAX_API_KEY`.
- Admin console for manual checks: `http://127.0.0.1:3000/admin` (sign in with `ORBITA_ADMIN_TOKEN`).
