...
- **Postgres schema bring-up has a locale gotcha — do NOT rely on the `docker compose` initdb mounts for schema.** In `pgvector/pgvector:pg16` the initdb glob sorts `002b_sessions_compression.sql` *before* `002_sessions.sql`, so the compression `ALTER TABLE sessions` runs before the table exists and initdb aborts (container exits code 3). The container still leaves a valid, initialized PGDATA, so a second `docker compose up -d postgres` starts Postgres serving (with only the `api_keys` table). Then apply the full schema explicitly, which is locale-independent: `DATABASE_URL=postgresql://orbita:orbita@localhost:5432/orbita pnpm db:migrate`. Note `db:migrate` omits two files — also apply `packages/lane-memory/drizzle/0003_notes.sql` and `packages/lane-scheduler/drizzle/0002_cron.sql` (all migrations are `IF NOT EXISTS`, safe to re-run). Expected result: 8 tables (`api_keys, sessions, messages, client_memories, notes, note_links, trajectory_events, session_jobs`).
