...
- Authoritative design spec: `usr/ORBITA_DESIGN.md`; W0–W35 shipped (w35 MCP OAuth); next Autopilot ship w36 notes export; wave roadmap `docs/development-plan.md`; Autopilot fuel `docs/autopilot/`; editorial poll `docs/at-editorial-poll.md`; Loop 2/4 deferred (H2/H3); `@orbita/harness` = Loop Engineering API (no `/v1/loops`); optional future `/v1/goals` semantic layer above harness; four-lane map in `docs/DEVELOPMENT_LANES.md`.
