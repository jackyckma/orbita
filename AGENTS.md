...
- Authoritative design spec: `usr/ORBITA_DESIGN.md`; W0–W34 in progress (w34 harness memory_inject + PA1 MCP `/v1/mcp`); wave roadmap `docs/development-plan.md`; editorial poll `docs/at-editorial-poll.md`; Loop 2/4 deferred (H2/H3); `@orbita/harness` = Loop Engineering API (no `/v1/loops`); optional future `/v1/goals` semantic layer above harness; four-lane map in `docs/DEVELOPMENT_LANES.md`.
...
- Personal steward PA0 (`docs/personal-steward/`, `client_id: personal-jacky`): cross-project knowledge hub; PA0 Cursor skill + `~/.orbita-personal.env`; PA1 MCP at `GET/POST /v1/mcp` (11 tools, Streamable HTTP, same Bearer + `x-orbita-client-id` auth).
