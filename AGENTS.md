...
- Authoritative design spec: `usr/ORBITA_DESIGN.md`; W0–W32 in progress (w27–w29 harness H1; w30 prompt date refresh; w31 `session_policy: per_run`, `GET /v1/memories/{key}`; w32 notes graph foundation); wave roadmap `docs/development-plan.md`; Loop 2/4 deferred (H2/H3); session profile snapshots immutable; four-lane map in `docs/DEVELOPMENT_LANES.md`.
- Not related to Jacky's OpenClaw orchestration or the public `openclaw/openclaw` repo.
- pnpm monorepo (`apps/orbita-api`, `apps/orbita-web`, `@orbita/*`); Postgres + pgvector via Drizzle; split deploy: production API on Zeabur at `https://api.get-orbita.com` (service ID `6a37d3a09f5fe35a4aa63552`, version track `0.0.1-w32`), marketing site `get-orbita.com` on Cloudflare Pages (static HTML, not Workers routes); prod `web_search` via self-hosted SearXNG at `https://orbita-searxng.zeabur.app` (`deploy/searxng/`, default `ORBITA_WEB_SEARCH_PROVIDER=searxng`).
