#!/usr/bin/env bash
# Verify portfolio git collector output: at least one repo-line report note within 48h.
#
# Usage:
#   ./scripts/portfolio-git-collect-verify-output.sh
#
...
if not notes:
    print(
        ...
        f"expected at least one with edge={expected_edge} (portfolio git collector may not have run).",
        file=sys.stderr,
    )
    sys.exit(1)
...
if not matches:
    print(
        ...
        "portfolio git collector output not verified.",
        file=sys.stderr,
    )
    sys.exit(1)

best = sorted(matches, key=lambda m: m.get("updated_at") or "", reverse=True)[0]
print("ok: portfolio git collector report verified")
